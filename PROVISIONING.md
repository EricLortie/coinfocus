# How to provision a new environment for cryptopredict
...
* Enter webroot path: `/home/deployer/apps/cryptopredict/shared/public/`

The `.pem` files created by `certbot-auto` need to be linked so Nginx can see them:

```
ln -s /etc/letsencrypt/live/example.com/fullchain.pem /etc/ssl/cryptopredict.crt
ln -s /etc/letsencrypt/live/example.com/privkey.pem /etc/ssl/cryptopredict.key
```
...
```
/opt/certbot/certbot-auto renew --webroot-path /home/deployer/apps/cryptopredict/shared/public/ --dry-run
```

Finally, add this to the root crontab to enable auto-renewal:

```
42 7,19 * * * /opt/certbot/certbot-auto renew --quiet --no-self-upgrade --webroot-path /home/deployer/apps/cryptopredict/shared/public/
```
